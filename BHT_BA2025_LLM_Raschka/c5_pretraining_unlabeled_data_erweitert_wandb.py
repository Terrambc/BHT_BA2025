...
### Zusatz: Funktion zur Berechnung der Accuracy
def calculate_accuracy(logits, targets):
    """Berechnet die Next-Token-Prediction Accuracy"""
    predictions = torch.argmax(logits, dim=-1)
    correct = (predictions == targets).float()
    # Ignoriere padding tokens (-100)
    mask = targets != -100
    if mask.sum() == 0:
        return 0.0
...
def train_model(model, train_loader, val_loader, optimizer, device, n_epochs,
                       ...
                       wandb_log=True, wandb_project=None,wandb_name=None, seed=123
                       ):
    ...
    ### Zusatz: Wandb Logging
    if wandb_log and wandb_project:

        # Berechne batch size und total batch size
        batch_size = next(iter(train_loader))[0].shape[0]
        seq_length = next(iter(train_loader))[0].shape[1]
        grad_accum_steps = 1  # Anpassen falls gradient accumulation verwendet wird
        total_batch_size = batch_size * seq_length * grad_accum_steps
        ...
        wandb.init(
            ...
            config={
                ...
                "max_steps": len(train_loader) * n_epochs,
                "num_epochs": n_epochs,
    ...
    peak_lr = optimizer.param_groups[0]["lr"] # ruft die inizialisierte Lernrate vom Optimizer ab
    ...
    for epoch in range(n_epochs):
        ...
        for input_batch, target_batch in train_loader:
            ### Zusatz für throughput Berechnung
            t0 = time.time()
        
            optimizer.zero_grad() # Verlustgradienten aus der vorherigen Batch-Iteration zurücksetzen
            global_step += 1

            # Passt die Lernrate an die aktuelle Phase an (Aufwärmphase oder Cosinus-Decay).
            ...
            ### Zusatz: Train Accuracy für den aktuellen Batch      
            with torch.no_grad():
                input_batch_device = input_batch.to(device)
                target_batch_device = target_batch.to(device)
                logits = model(input_batch_device)
                train_accuracy = calculate_accuracy(
                    logits.flatten(0, 1), 
                    target_batch_device.flatten()
                )       
            ...
            ### Zusatz Zeit und throughput berechnung
            t1 = time.time()
            dt = t1 - t0
            tokens_per_sec = input_batch.numel() / dt

            ### Zusatz: Debug-Output
            if global_step % 100 == 0:  # Alle 100 Steps
                print(f"Step {global_step}, Zeit: {time.time() - t0:.2f}s")
            ...
            if global_step % eval_freq == 0:
                ...
                ### Zusatz: Berechnung von Validation accuracy
                model.eval()
                val_accuracy_accum = 0.0
                val_batches = 0
                with torch.no_grad():
                    for val_input, val_target in val_loader:
                        if val_batches >= eval_iter:
                            break
                        val_input, val_target = val_input.to(device), val_target.to(device)
                        val_logits = model(val_input)
                        val_accuracy_accum += calculate_accuracy(
                            val_logits.flatten(0, 1), 
                            val_target.flatten()
                        )
                        val_batches += 1
                val_accuracy = val_accuracy_accum / val_batches if val_batches > 0 else 0.0
                model.train()

                ### Zusatz: Berechnet Perplexity
...
# Berechnung von Verlusten über den Trainings- und Validierungssatz
...
def main():
    ...
    ### Zusatz: Angepasst an Karpathy, verkürzung der Trainingszeit für mehr runden
    GPT_CONFIG_124M = {

        "vocab_size": 50257,    #  # Vokabelgröße - in dem Fall 50527 Wörter
        "context_length": 512, # maximale Anzahl von Input Tokens das Model kann händeln via dem Positional Embeddings
        "emb_dim": 384,         # Größe des Embeddings - jeder Token wird in einen 768 dimensionalen Vektor umgewandelt
        "n_heads": 6,          # Anzahl der Attention Heads im Multi-head Attention Mechanismus
        "n_layers": 6,         # Anzahl von Transformer Blöcken
