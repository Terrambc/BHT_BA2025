...
- Optional: DOT-Graph der Importe (auch externe Pakete darstellbar)
...
def main():
    ...
    ap.add_argument('--dot', dest='dot_out', help='Optional: Graphviz-DOT-Datei der Importe')
    ap.add_argument('--dot-include-external', action='store_true',
                    help='Externe Pakete in DOT aufnehmen (graue Ellipsen, gestrichelte Kanten)')
    ap.add_argument('--json', dest='json_out', help='Optional: JSON-Sammelausgabe')
    ap.add_argument('--top', type=int, default=5, help='Top-N Hotspot-Funktionen (Standard: 5)')
    ...
    for m, mi in project.modules.items():
        sloc = mi.sloc
        ext_set = external_imports.get(m, set())
        ext = len(ext_set)
    ...
    # Externe Pakete als CSV (für Komponentendiagramm)
    external_rows = []
    for mod, pkgs in external_imports.items():
        for pkg in sorted(pkgs):
            external_rows.append({"module": mod, "external_package": pkg})
    ...
    # CSV: external_packages.csv
    with (outdir / "external_packages.csv").open("w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow(["module", "external_package"])
        for r in external_rows:
            w.writerow([r["module"], r["external_package"]])

    print(f"CSV-Tabellen in {outdir} geschrieben.")

    # DOT (optional)
    if args.dot_out:
        with Path(args.dot_out).open("w", encoding="utf-8") as fh:
            fh.write("digraph imports {  rankdir=LR;  node [shape=box];")
            # Projektmodule als Knoten (auch ohne Kanten)
            for src in internal_graph.keys():
                fh.write(f'  "{src}" [shape=box];')
            # Interne Kanten
            for src, dsts in internal_graph.items():
                for dst in dsts:
                    fh.write(f'  "{src}" -> "{dst}";')
            # Optional: externe Pakete als Ellipsen mit gestrichelten Kanten
            if args.dot_include_external:
                for mod, pkgs in external_imports.items():
                    for pkg in sorted(pkgs):
                        fh.write(f'  "{pkg}" [shape=ellipse, style=dashed, color=gray];')
                        fh.write(f'  "{mod}" -> "{pkg}" [style=dashed, color=gray];')
                        fh.write("}")
